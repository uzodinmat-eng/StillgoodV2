...
/**
 * Marks a wallet-paid order as successful: the payment row is settled and the
 * order moves straight into store confirmation. Money already left the
 * customer's wallet via debitWallet before this runs.
 */
export async function markWalletPaymentSuccessful(paymentId: string, amountNaira: number, orderId: string): Promise<void> {
  await execute(
    `update public.order_payments
     set status = 'success', paid_at = now(), updated_at = now()
     where id = $1 and status = 'pending'`,
  ...
  await execute(
    `update public.orders
     set status = 'awaiting_store_confirmation', updated_at = now()
     where id = $1 and status = 'pending_payment'`,
    [orderId]
  );
  // The customer wallet row must exist before ledger entries can reference it
  // (ledger_entries.wallet_id has a foreign key to wallets).
  await execute(
    `insert into public.wallets (id, owner_type, owner_id)
     select 'wallet_customer_' || customer_id, 'customer', customer_id
     from public.orders where id = $1
     on conflict (id) do nothing`,
    [orderId]
  );
  const txnId = `txn_wallet_${orderId}`;
  await execute(
    `insert into public.ledger_entries (txn_id, wallet_id, amount, kind, ref_type, ref_id, note)
     values ($1, 'wallet_customer_' || (select customer_id from public.orders where id = $2), $3, 'payment_in', 'order_payment', $4, 'Paid with Stillgood Wallet')
     on conflict (txn_id, wallet_id, kind) do nothing`,
    [txnId, orderId, -amountNaira, paymentId]
  );
  await execute(
    `insert into public.ledger_entries (txn_id, wallet_id, amount, kind, ref_type, ref_id, note)
     values ($1, 'wallet_platform', $2, 'payment_in', 'order_payment', $3, 'Paid with Stillgood Wallet')
     on conflict (txn_id, wallet_id, kind) do nothing`,
    [txnId, amountNaira, paymentId]
  );
}
